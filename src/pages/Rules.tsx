...
const SLUGS = ["rules_intro", "rules_adjusted", "rules_rejected", "rules_closing"];

const Rules = () => {
  const { get } = useSitePages(SLUGS);
  const intro = get("rules_intro");
  ...
  const sections: { slug: string; list: boolean }[] = [
    { slug: "rules_adjusted", list: true },
    { slug: "rules_rejected", list: true },
    { slug: "rules_closing", list: false },
  ];
