/**
 * MARKDOWN LINK PARSER (browser / app side).
 *
 * Mirrored verbatim in
 * `supabase/functions/_shared/transactional-email-templates/markdownLinks.ts`
 * for the email templates. Any change here MUST be mirrored there. A true
 * shared import is impossible: the edge copy runs in Deno inside
 * `supabase/functions/`, which cannot reach into `src/`.
 *
