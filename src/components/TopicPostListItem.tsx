...
const TopicPostListItem = ({ post, rank, topicName, topicId, showRanking = true }: TopicPostListItemProps) => {
  ...
  const goToPost = () => {
    const slug = buildPostSlug(displayTitle, post.id) || String(rank);
  ...
  return (
    ...
    >
      ...
      {showRanking && (
        <>
          <span className="shrink-0 w-10 ml-4 text-right text-base md:text-lg font-heading text-secondary font-semibold tabular-nums">
            {seedAvg}
          </span>
